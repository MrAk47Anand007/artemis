...
│   └── manual_mode.py    # mobile_observe / mobile_act / mobile_session_end
...
* **`mobile_observe` / `mobile_act` / `mobile_session_end`**: Host-agent-driven manual mode. Unlike `mobile_run_task`, these tools perform no reasoning of their own and need **no LLM credential configured in Artemis** — the calling MCP host (Claude Code, Codex, Antigravity, or any other host with its own model) observes the screen (`mobile_observe`), decides the next step itself, and issues it through `mobile_act` (`click`, `long_press`, `input_text`, `click_sequence`, `swipe`, `press_key`, `manage_app`, `wait_for_delay`, `wait_for_text`, `open_link`, `erase_one_char`, `focus_and_clear_text`) — the same reactive loop `FlashRunner` runs autonomously, just driven turn-by-turn by the host. A `click`/`long_press`/`input_text` `target` may be an element index from the last observation's element list, or a normalized `[x, y]` pair with a `target_description`. Sessions are keyed by `device_serial`, guarded by the same per-device lock `mobile_run_task` uses (so a manual session and an autonomous task can't run on one device at once), and auto-released after 10 minutes idle if `mobile_session_end` isn't called explicitly. `ask_explorer`/`video_analyzer`/note tools are intentionally unavailable here since they invoke Artemis's own LLM router internally. See [`docs/superpowers/specs/2026-09-16-host-agent-manual-mode-design.md`](../docs/superpowers/specs/2026-09-16-host-agent-manual-mode-design.md) for the full design.
